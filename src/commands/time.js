		...
		commons.getWfStatInfo(config.WFStatApiURL + argsFound.valid.cycle.path).then((timeData) => {
            switch(argsFound.valid.cycle.path) {
                case '/cambionCycle':
                    ...
                    return message.reply(`\nIt is currently **${time}** on ${argsFound.valid.cycle.name} (${commons.timeLeftWithSecMsgFormat(timeData.expiry)} left)`);
                default:
                    return message.reply(`\nIt is currently **${timeData.state}** on ${argsFound.valid.cycle.name} (${commons.timeLeftWithSecMsgFormat(timeData.expiry)} left)`);
              }  
		})
	},
};