...
module.exports = {
  ...
  execute(message, args) {
    ...
    commons.getWfStatInfo(`${config.WFStatApiURL}/sortie`).then((sortieData) => {
      const filteredReply = this.notification(sortieData, argsFound.valid);
  ...
  notification(sortieData, args, lastCheckedDate) {
    ...
    if ((lastCheckedDate === undefined || (lastCheckedDate <= Date.parse(sortieData.activation)))) {
      if (sortieData.variants.some(
        (sortieMission) => String(sortieMission.missionType).toLowerCase() === args.mission,
      )) {
        return null;
      }
